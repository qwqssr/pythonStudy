...
class HumanMouseSimulator:
    ...
    def _generate_base_trajectory(self, start_pos: Tuple[float, float],
        ...
        if trajectory_type == "bezier":
            return self._generate_bezier_trajectory(start_pos, end_pos, duration)
        elif trajectory_type == "arc":
            return self._generate_arc_trajectory(start_pos, end_pos, duration)
        else:  # direct with curves
            return self._generate_curved_direct_trajectory(start_pos, end_pos, duration)
...
class Actions:
    ...
    def move_to(self, ele_or_loc, offset_x=None, offset_y=None, duration=.5):
        ...
        # 使用人类鼠标模拟器生成轨迹
        trajectory = self.human_mouse.generate_trajectory((self.curr_x, self.curr_y), (cx, cy), duration)
        ...
        for point in trajectory:
            ...
            # 根据真实时间间隔休眠
            if len(trajectory) > 1 and point != trajectory[0]:
                prev_point = trajectory[trajectory.index(point) - 1]
                sleep_time = point.timestamp - prev_point.timestamp
                if sleep_time > 0:
                    sleep(sleep_time)
    ...
    def move(self, offset_x=0, offset_y=0, duration=.5):
        """保持原有接口，但使用人类鼠标模拟器"""
        target_x = self.curr_x + offset_x
        target_y = self.curr_y + offset_y

        # 使用人类鼠标模拟器生成轨迹
        trajectory = self.human_mouse.generate_trajectory((self.curr_x, self.curr_y), (target_x, target_y), duration)
